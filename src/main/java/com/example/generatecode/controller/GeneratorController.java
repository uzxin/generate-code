package com.example.generatecode.controller;

import com.alibaba.fastjson.JSON;
import com.example.generatecode.controller.utils.GenUtils;
import com.example.generatecode.controller.utils.R;
import com.example.generatecode.service.GeneratorService;
...
public class GeneratorController {
	...
	public void batchCode(HttpServletRequest request, HttpServletResponse response, String tables) throws IOException {
		...
		byte[] data = generatorService.generatorCode(tableNames);
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\"yueche.zip\"");
		response.addHeader("Content-Length", "" + data.length);
		response.setContentType("application/octet-stream; charset=UTF-8");
